...
static int major;
static struct cdev backlight_dev;
...
static int backlight_probe(struct platform_device *pdev)
{
	...
	if(!one_write_pin) {
		printk("of_get_named_gpio return error\n");
		return -EINVAL;
	}
	...
	major = MAJOR(devid);
	cdev_init(&backlight_dev, &backlight_fops);
	cdev_add(&backlight_dev, devid, 1);
...
static int backlight_init(void) 
{
	...
	if(ret) {
		printk(KERN_ERR "backlight demo: probe failed backlight:%d\n", ret);
	}
